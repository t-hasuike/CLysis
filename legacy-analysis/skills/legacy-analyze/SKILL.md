...
## 前提条件
- input/project/ にプロジェクト基本情報（overview.md, repositories.md, tech_stack.md）が存在すること
- input/project/diagrams/ に図の管理場所が存在すること
...
4. ドメイン知識をinput/domain/に永続化（書記が担当）
5. 「わかったこと/わからないこと」リストを更新
6. 出力:
   - input/project/diagrams/ の3つの図を更新
   - input/domain/ に新規知識を追加
...
| Knowledge Graph Memory | input/domain/ + diagrams/ | Phase 3 |
...
| 種別 | 形式 | 出力先 |
|------|------|--------|
| Phase 0 成果物 | overview.md + システム鳥瞰図 + わかったこと/わからないことリスト | `input/project/overview.md`, `input/project/diagrams/` |
| Phase 1 成果物 | 影響分析レポート + Service仕様 + DFD断片 + わからないことリスト更新 | `output/`, `input/project/diagrams/` |
| Phase 2 成果物 | 監査済み成果物（修正反映済み） | `output/` (updated) |
| Phase 3 成果物 | 更新された3つの地図 + ドメイン知識 | `input/project/diagrams/`, `input/domain/` |
...
- [ ] Phase 3: ドメイン知識をinput/domain/に永続化したか
